...
const PLAIN_COMMENT_BTN_ID = "add_comment_btn";
...
function addCriterion(criterion) {
    ...
    document
        .getElementById("criteria_list_container")
        .appendChild(criterionNode);
}
...
function copyBtnHandler() {
    const appliedCriteria = document.getElementById(MARKS_TEXTAREA_ID).value;
...
function applyClickHandlers() {
    ...
    document.getElementById(PLAIN_COMMENT_BTN_ID).onclick = addPlainCommentBtnHandler;
}

function updateTotalGrade(delta) {
    const gradeP = document.getElementById("grade_p");
    gradeP.textContent = parseFloat(gradeP.textContent) + delta;
}
...
function applyCriterion(criterion) {
    return () => {
        ...
        textarea.value += `${criterion.value} ${criterion.description}${comments}\n`;
        updateTotalGrade(parseFloat(criterion.value));
    };
}
...
function removeCriterion(criterion) {
    return () => {
        ...
        const index = appliedCriteria.findIndex((appliedCriterion) => {
            let valueLength = criterion.value.toString().length;

            const candidate = appliedCriterion.substring(
                0,
                criterion.description.length + valueLength + 1
            );
            return (
                candidate.normalize() ===
                `${criterion.value} ${criterion.description}`.normalize()
            );
        });
        ...
        updateTotalGrade(0 - parseFloat(criterion.value));
    };
}

function addPlainCommentBtnHandler() {
    const textarea = document.getElementById(MARKS_TEXTAREA_ID);
    const commentsInput = document.getElementById("comments_input");
    let comments = commentsInput.value;
    commentsInput.value = "";
